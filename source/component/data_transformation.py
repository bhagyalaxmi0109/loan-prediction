...
class DataTransformation:
    ...
    def oversample_smote(self, data):
        try:
            ...
            return pd.concat(
                [pd.DataFrame(x_resmapled, columns=x.columns), pd.DataFrame(y_resampled, columns=['Loan_Status'])],
                axis=1)
    ...
    def initiate_data_transformation(self, key):
        ...
        if key == 'train':
            ...
            train_data = self.feature_encoding(train_data, target='Loan_Status', save_encoder_path=self.utility_config.dt_multi_class_encoder, key='train')
            test_data = self.feature_encoding(test_data, target='Loan_Status', load_encoder_path=self.utility_config.dt_multi_class_encoder, key='test')
            ...
            export_data_csv(train_data, self.utility_config.train_file_name,
                            self.utility_config.train_dt_train_file_path)
        ...
        if key == 'predict':
            ...
            predict_data = self.feature_encoding(predict_data, target='',
                                                 load_encoder_path=self.utility_config.dt_multi_class_encoder,
                                                 key='predict')
